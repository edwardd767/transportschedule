...
function BookingStatusReport({ bookings, from, to, onFrom, onTo, onBack }: { bookings: Booking[]; from: string; to: string; onFrom: (value: string) => void; onTo: (value: string) => void; onBack: () => void }) {
  const rows = bookings.filter(item => item.arrival >= from && item.arrival <= to);
  const totals = rows.reduce((value, item) => ({ adults: value.adults + item.rooms.reduce((sum, room) => sum + (room.adults ?? item.guests ?? 0), 0), children: value.children + item.rooms.reduce((sum, room) => sum + (room.children ?? 0), 0), rooms: value.rooms + item.rooms.reduce((sum, room) => sum + room.count, 0), amount: value.amount + item.amount }), { adults: 0, children: 0, rooms: 0, amount: 0 });
  return <div className="booking-status-report"><div className="report-view-head"><button type="button" onClick={onBack}>‹ Back to reports</button><strong>Booking Status Report</strong></div><div className="report-date-filter"><label>Booking Start Date<input type="date" value={from} onChange={event => onFrom(event.target.value)} /></label><label>Booking End Date<input type="date" value={to} onChange={event => onTo(event.target.value)} /></label></div><div className="report-table-wrap"><table><thead><tr>{['Booking No','Booking Date','Guest Name','Guest Type','Adult','Child','Room No','Room Type','Rate Code','Room Rate','Disc','Nett','Arrival Date','Departure Date','Night','Status','Reference No','Billing Instruction','Special Request','Promo Code','Deposit','Audit User','Date / Time'].map(item => <th key={item}>{item}</th>)}</tr></thead><tbody>{rows.map(item => { const room = item.rooms[0]; const nights = Math.max(1, Math.round((Date.parse(item.departure) - Date.parse(item.arrival)) / 86400000)); return <tr key={item.reference}><td>{item.reference}</td><td>{item.arrival}</td><td>{item.guest}</td><td>{item.guestType ?? 'N'}</td><td>{room?.adults ?? item.guests}</td><td>{room?.children ?? 0}</td><td>{room?.count ?? 0}</td><td>{room?.code ?? ''}</td><td>{room?.rateCode ?? ''}</td><td>{(room?.roomRate ?? item.amount).toFixed(2)}</td><td>{(room?.discount ?? 0).toFixed(2)}</td><td>{item.amount.toFixed(2)}</td><td>{item.arrival}</td><td>{item.departure}</td><td>{nights}</td><td>{item.status}</td><td>{item.referenceNo ?? ''}</td><td>{item.billingRemark ?? ''}</td><td>{item.specialRequests ? Object.values(item.specialRequests).join(', ') : ''}</td><td>{room?.promoCode ?? ''}</td><td>0.00</td><td>HotelX</td><td>{item.arrival}</td></tr>; })}</tbody></table></div><div className="report-total-line">Total Adult/Child: {totals.adults}/{totals.children} &nbsp;&nbsp; Total Room: {totals.rooms} &nbsp;&nbsp; Total Amount: {totals.amount.toFixed(2)}</div></div>;
}
