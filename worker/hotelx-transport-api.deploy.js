  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'rateSetup', jsonb_build_object(
        ...
        'validity', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', v.id, 'rateSetupId', v.rate_setup_id, 'from', to_char(v.valid_from, 'YYYY-MM-DD'), 'to', to_char(v.valid_to, 'YYYY-MM-DD'), 'active', v.active, 'seasonalRates', v.seasonal_rates, 'inclusiveElements', COALESCE((SELECT jsonb_agg(ve.rate_element_id::text) FROM public.hotelx_rate_setup_validity_element ve WHERE ve.property_id = v.property_id AND ve.validity_id = v.id), '[]'::jsonb)) ORDER BY v.sort_order) FROM public.hotelx_rate_setup_validity v WHERE v.property_id = meta.id), '[]'::jsonb)
      ),
