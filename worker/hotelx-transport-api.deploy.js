...
function applyTransportAction(state, input) {
  ...
  switch (action.type) {
    ...
    case "status": {
      ...
      if (action.status === "Completed" && current.groups.some((g) => !g.boarded))
        throw new Error(
...
var readSql = "SELECT revision::text, state::text FROM public.hotelx_transport_state WHERE id = $1";
var insertSql = "INSERT INTO public.hotelx_transport_state (id, state) VALUES ($1, $2::jsonb) ON CONFLICT (id) DO NOTHING RETURNING revision::text, state::text";
var updateSql = "UPDATE public.hotelx_transport_state SET state = $1::jsonb, revision = revision + 1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND revision = $3::integer RETURNING revision::text";
