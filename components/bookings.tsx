...
function BookingOccupancy({ booking }: { booking: Booking }) {
  return (
    <span className="booking-occupancy">
      <span
        aria-label={`${booking.assignedRooms} of ${roomCount(booking)} rooms assigned`}
      >
        <DoorClosed size={18} aria-hidden="true" />
        <span
          className={
            booking.assignedRooms < roomCount(booking)
              ? 'booking-incomplete'
              : ''
          }
        >
      ...
      <span
        aria-label={`${booking.checkedInGuests} of ${booking.guests} guests checked in`}
      >
        <UserRound size={18} aria-hidden="true" />
        <span
          className={
            booking.checkedInGuests < booking.guests ? 'booking-incomplete' : ''
          }
        >
    ...
    if (booking) {
      ...
      listRef.current
        .querySelector<HTMLButtonElement>(
          `[data-booking="${lastBooking.current}"]`,
        )
      ...
      <section className="booking-workspace" aria-label="Booking details">
        ...
        <div className="booking-detail-summary">
          ...
          <div className="booking-detail-bottom">
            <span>
              {booking.reference} <span className="booking-divider">|</span>{' '}
              {booking.guest}
            </span>
            ...
            <button
              ...
              onClick={() =>
                section.title === 'Transport'
                  ...
                  : onNotice(
                      `${section.title} is shown for reference. Editing hotel bookings is not included in this preview.`,
                    )
              }
            >
              <span>
  ...
  return (
    <section className="booking-workspace" aria-label="Booking listing">
      <div className="listing-title booking-listing-title">
        <h1>
          Booking Listing <span>({shown.length})</span>
        </h1>
        <div className="booking-toolbar">
          ...
          <button
            ...
            title={
              oldestFirst ? 'Oldest bookings first' : 'Newest bookings first'
            }
          ...
          {(searchOpen || query) && (
            <label className="search-field">
              ...
              <input
                ...
                aria-label="Search booking reference or guest"
          ...
          {(filtersOpen || status !== 'all') && (
            <Choice
              ...
              items={[
                ...
                ...bookingStatuses.map((value) => ({ value, label: value })),
              ]}
            />
          )}
      ...
      <div className="booking-legend" aria-label="Booking statuses">
        {bookingStatuses.map((item) => (
          <span key={item}>
            <i className={bookingStatusClass(item)} />
            {item}
          </span>
        ))}
      </div>
      <div className="booking-list-scroll" ref={listRef}>
        <div className="booking-list">
          {shown.map((item) => (
            <button
              className={`booking-row ${bookingStatusClass(item.status)}`}
              key={item.reference}
              data-booking={item.reference}
              onClick={() => openBooking(item)}
              aria-label={`Open booking ${item.reference} for ${item.guest}, ${item.status}`}
            >
              <div className="booking-copy">
                <div className="booking-stay">
                  <strong
                    className={item.highlightDates ? 'booking-highlight' : ''}
                  >
                    {stayDates(item)}
                  </strong>
                  <BookingOccupancy booking={item} />
                </div>
                <span className="booking-guest">
                  {item.reference} <span className="booking-divider">|</span>{' '}
                  {item.guest}
                </span>
              </div>
          ...
          {!shown.length && (
            <div className="empty-state">
              ...
              <p>Try another guest name, status or arrival date.</p>
              <button className="secondary-button" onClick={resetFilters}>
                Clear filters
              </button>
            </div>
      ...
      <button
        ...
        title="Add booking"
        onClick={() =>
          onNotice('Booking creation is not included in this preview.')
        }
      >
        <Plus size={28} />
      </button>
