...
const metricColumns = ['Room', 'Occ(%)', 'Guest', 'Child', 'Room Rev.', 'Other Rev.', 'FNB Rev.', 'ARR'];
const cellFormats = ['number', 'money', 'number', 'number', 'money', 'money', 'money', 'money'];
...
function production(rows: Booking[], date: string, scope: Scope, roomTotal: number) {
  ...
  return { room, occ, guest, child, roomRev, otherRev: 0, fnbRev: 0, arr: room ? roomRev / room : 0 };
}
...
function buildRows(bookings: Booking[], date: string, roomTotal: number) {
  ...
  return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([segment, rows]) => {
    const cells = periods.flatMap((scope) => {
      ...
      return [value.room, value.occ, value.guest, value.child, value.roomRev, value.otherRev, value.fnbRev, value.arr];
    });
...
function SegmentRow({ label, cells, total }: { label: string; cells: number[]; total?: boolean }) {
