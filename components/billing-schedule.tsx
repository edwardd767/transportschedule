...
import { roomAssignments, roomCount, type BillingScheduleAdjustment, type Booking, type BookingRoom } from '@/lib/bookings';
...
export function BillingSchedule({ booking, bookingLegs, rateSetup, onSave, onBack }: { booking: Booking; bookingLegs: BookingTransportLeg[]; rateSetup: RateSetupData; onSave: (booking: Booking) => Promise<void>; onBack: () => void }) {
  ...
  const totalRoomCount = roomCount(booking);
  const inhouse = booking.status === 'Inhouse';
  const checkedInRooms = inhouse ? Math.min(booking.assignedRooms, totalRoomCount) : 0;
  ...
  return <section className="booking-workspace billing-schedule billing-schedule-page" aria-label="Billing schedule">
    ...
    <div className="booking-detail-summary billing-schedule-summary">
      <div className="booking-detail-top"><div className="booking-stay"><strong>{stayLabel(booking.arrival)} - {stayLabel(booking.departure)}</strong><span><RoomIcon size={14} /> {checkedInRooms}/{totalRoomCount}&nbsp;&nbsp; <PersonIcon size={14} /> {checkedInGuests}/{booking.guests}</span></div><strong className="booking-amount">{money(roomTotal + transportTotal)}</strong></div>
            ...
            return <div className="billing-room-block" key={roomKey}>
              ...
              {isRoomOpen && <div className="billing-room-detail">
                ...
                {(() => {
                  ...
                  const seasonName = rateSetup?.seasons.find((item) => item.id === rateSetup.calendar[line.date])?.name;
                  const nightlyRate = bookingRate(rateSetup, line.rateCode, line.roomTypeCode, line.date)?.amount ?? line.baseAmount;
                  const chargeLines = [
                    { label: 'Room Charge', amount: Math.max(0, roomCharge - line.extraPax) },
                    ...(line.extraPax > 0 ? [{ label: 'Extra Pax', amount: line.extraPax }, ...line.extraPaxLines.map((entry) => ({ ...entry, muted: true }))] : []),
                  ];
                  const rateNote = nightlyRate !== undefined ? `Rate: ${money(nightlyRate)}${seasonName ? ` · ${seasonName}` : ''}` : undefined;
                  const standardBreakdown: { key: string; name: string; amount: number; info?: { title: string; note?: string; lines: { label: string; amount: number; muted?: boolean }[]; total?: number } }[] = [
                    { key: 'room-charge', name: 'Room Charge', amount: roomCharge, info: rateNote || chargeLines.length > 1 ? { title: 'Breakdown', note: rateNote, lines: chargeLines, total: roomCharge } : undefined },
                    ...line.elements.filter((item) => item.amount !== 0).map((item, index) => ({ key: `element-${index}`, name: item.name, amount: item.amount })),
