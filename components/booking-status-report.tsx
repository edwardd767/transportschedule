...
const columns = ['Booking No', 'Booking Date', 'Guest Name', 'Guest Type', 'Corp/TA', 'A/C/I', 'Nationality', 'Segment', 'Room No', 'Room Type', 'Rate Code', 'Room Rate', 'Disc', 'Nett Rate', 'Arrival Date', 'Departure Date', 'Night', 'Status', 'Reference No', 'Billing Instruction', 'Special Request', 'Promo Code', 'Deposit', 'Audit User', 'Date / Time'];
...
export function BookingStatusReport({ bookings, profiles, hotelName, from, to, onFrom, onTo, onBack }: {
  ...
  const infants = (item: Booking) => item.rooms.reduce((sum, room) => sum + (room.infants ?? 0) * room.count, 0);
  ...
  return <div className="booking-status-report">
    ...
    <div className="report-table-wrap" tabIndex={0} role="region" aria-label="Booking status report, scroll to view all columns"><table><thead><tr>{columns.map(item => <th scope="col" key={item}>{item}</th>)}</tr></thead><tbody>
      {rows.map(item => {
        ...
        const values = [item.reference, '—', item.guest, join(guests.map(profile => profile.guestType)), item.accountName || '—', `${adults(item) ?? '—'}/${children(item)}/${infants(item)}`, join(guests.map(profile => profile.nationality)), item.segment || '—', '—', item.rooms.map(room => `${room.code} × ${room.count}`).join(', '), join([...item.rooms.map(room => room.rateCode), ...(item.billingSchedule ?? []).map(line => line.rateCode)]),
