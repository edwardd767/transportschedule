...
const columns = ['Booking No', 'Booking Date', 'Guest Name', 'Guest Type', 'Corp/TA', 'Adult', 'Nationality', 'Segment', 'Room No', 'Room Type', 'Rate Code', 'Room Rate', 'Disc', 'Nett Rate', 'Arrival Date', 'Departure Date', 'Night', 'Status', 'Reference No', 'Billing Instruction', 'Special Request', 'Promo Code', 'Deposit', 'Audit User', 'Date / Time'];
...
export function BookingStatusReport({ bookings, profiles, hotelName, from, to, onFrom, onTo, onBack }: {
  ...
  return <div className="booking-status-report">
    ...
    <p className="booking-report-note">Scroll horizontally to view all 25 columns. “—” means no saved value. Booking date, assigned room number, deposit and audit details are not currently supplied by the booking records; the date filter uses arrival dates.</p>
    <div className="report-table-wrap" tabIndex={0} role="region" aria-label="Booking status report, scroll to view all columns"><table><thead><tr>{columns.map(item => <th scope="col" key={item}>{item}</th>)}</tr></thead><tbody>
      {rows.map(item => {
        ...
        const values = [item.reference, '—', item.guest, join(guests.map(profile => profile.guestType)), item.accountName || '—', adults(item) ?? '—', join(guests.map(profile => profile.nationality)), item.segment || '—', '—', item.rooms.map(room => `${room.code} × ${room.count}`).join(', '), join([...item.rooms.map(room => room.rateCode), ...(item.billingSchedule ?? []).map(line => line.rateCode)]),
    ...
    <div className="report-total-line">Total Adult/Child: {rows.every(item => adults(item) !== undefined) ? rows.reduce((sum, item) => sum + (adults(item) ?? 0), 0) : '—'}/{rows.reduce((sum, item) => sum + children(item), 0)} &nbsp; Total Room: {totalRooms} &nbsp; Nett Total: {money(totalAmount)}</div>
