...
export function BookingAvailability({ arrival, bookings, roomTypes, rateSetup }: { rateSetup?: RateSetupData; arrival: string; bookings: Booking[]; roomTypes: HotelRoomType[] }) {
  ...
  const rateFor = (planId: string, date: string) => {
    ...
    return validity?.seasonalRates?.[selectedRoom.code]?.[season]?.amount;
  };
