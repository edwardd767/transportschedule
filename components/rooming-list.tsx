  ...
  if (page === 'profile' && draft) return <section className="rooming-page" aria-label="Guest profile details">
    ...
    <div className="rooming-profile-card">
      ...
      <div className="rooming-form-grid">
        {([['name', 'Guest Name *'], ['mobile', 'Mobile No.'], ['email', 'Email Address'], ['nationality', 'Nationality *'], ['identityNo', 'NRIC No.'], ['address', 'Address'], ['country', 'Country *'], ['state', 'State *'], ['city', 'City'], ['postcode', 'Postcode'], ['vehicle', 'Vehicle No. / Model'], ['occupation', 'Occupation'], ['birthDate', 'Birth Date'], ['guestType', 'Guest Type *'], ['adultChild', 'Adult/Child *'], ['remark', 'Remark'], ['paymentRemark1', 'PaymentRemark1'], ['paymentRemark2', 'PaymentRemark2'], ['taxExemptReason', 'Tax Exempted Reason Code']] as const).map(([key, label]) => <label className={['name','mobile','email','address','paymentRemark1','paymentRemark2','taxExemptReason'].includes(key) ? 'rooming-wide' : ''} key={key}><span>{label}</span>{key === 'adultChild' ? <select value={draft.adultChild || 'Adult'} onChange={event => setDraft({ ...draft, adultChild: event.target.value as 'Adult' | 'Child' | 'Infant' })}><option>Adult</option><option>Child</option><option>Infant</option></select> : key === 'country' ? <select value={draft.country} onChange={event => updateDraft(key, event.target.value)}>{geography.countries.map(item => <option key={item}>{item}</option>)}</select> : key === 'state' ? <select value={draft.state} onChange={event => updateDraft(key, event.target.value)}><option value="">Select state</option>{locationStates.map(item => <option key={item}>{item}</option>)}</select> : key === 'city' ? <select value={draft.city} onChange={event => updateDraft(key, event.target.value)}><option value="">Select city</option>{locationCities.map(item => <option key={item}>{item}</option>)}</select> : key === 'nationality' && geography.nationalities.length ? <select value={draft.nationality} onChange={event => updateDraft(key, event.target.value)}>{draft.nationality && !geography.nationalities.includes(draft.nationality) ? <option>{draft.nationality}</option> : null}{geography.nationalities.map(item => <option key={item}>{item}</option>)}</select> : <input type={key === 'birthDate' ? 'date' : key === 'email' ? 'email' : 'text'} value={draft[key] ?? ''} onChange={event => updateDraft(key, event.target.value)} />}</label>)}        <label className="rooming-check"><input type="checkbox" checked={draft.tourismTax} onChange={event => setDraft({ ...draft, tourismTax: event.target.checked })} /><span>Tourism Tax</span></label>
      </div>
    </div>
