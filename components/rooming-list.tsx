...
import { Baby, ChevronDown, ChevronUp, ClipboardList, Pencil, Plus, ScanLine, ContactRound, UserRound, UserRoundPen } from 'lucide-react';
import { InfantIcon } from '@/components/infant-icon';
import { PersonIcon } from '@/components/person-icon';
import { useContext, useEffect, useMemo, useState } from 'react';
import { Dialog, DialogContent, DialogTitle } from '@/components/ui/dialog';
import { roomAssignments, stayDates } from '@/lib/bookings';
...
export function RoomingList({ booking, profiles, onProfilesSave, onBookingSave, onBack, paxCountPolicy, rateSetup, childAgePolicy = 0 }: {
  ...
  const removeProfile = async () => {
    if (!draft || assigned.length <= 1) return;
  ...
  if (page === 'guests') return <section className="rooming-page" aria-label="Room guest list">
    ...
    <div className="rooming-booking-line"><strong>{booking.reference} | {booking.guest}</strong><small>{roomLabel}</small></div>
    <div className="rooming-pax-head"><span><PersonIcon size={16} /> No. of Pax: <b>{pax}</b> <b className="rooming-pax-count"><span className="booking-pax-icon billing-pax-tip" data-tip="Adult"><UserRound size={15} aria-label="Adults" /></span> {room?.adults ?? 1} <span className="booking-pax-icon billing-pax-tip" data-tip="Child"><Baby size={15} aria-label="Children" /></span> {room?.children ?? 0} <span className="booking-pax-icon billing-pax-tip" data-tip="Infant"><InfantIcon size={15} label="Infants" /></span> {room?.infants ?? 0}</b></span><button type="button" aria-label="Edit number of pax" onClick={() => { setAdultDraft(String(room.adults ?? 1)); setChildDraft(String(room.children ?? 0)); setInfantDraft(String(room.infants ?? 0)); setPaxOpen(true); setError(''); }}><Pencil size={17} /></button></div>
    <Dialog open={paxOpen} onOpenChange={setPaxOpen}><DialogContent className="rooming-pax-dialog" aria-describedby={undefined} showCloseButton={false}><DialogTitle>{booking.reference} | {booking.guest}</DialogTitle><small>{roomLabel}</small><form onSubmit={event => { event.preventDefault(); void run(async () => { const adults = Number(adultDraft), children = Number(childDraft), infants = Number(infantDraft); if (![adults,children,infants].every(n => Number.isInteger(n) && n >= 0) || adults + children < 1) throw new Error('Enter adult, child and infant counts with at least one guest.'); const rooms = booking.rooms.map((item,index) => index === roomIndex ? { ...item, adults, children, infants } : item); await saveRooms(rooms); setPaxOpen(false); }); }}><div className="booking-guests-steppers rooming-guests-steppers"><div className="booking-stepper"><div className="booking-stepper-row"><span className="booking-stepper-label">Adult</span><input className="booking-stepper-value" type="number" min={1} step={1} required aria-label="No. of Adult" value={adultDraft} onChange={e => setAdultDraft(e.target.value)} /><span className="booking-stepper-arrows"><button type="button" aria-label="Increase adults" onClick={() => setAdultDraft(String(Math.floor(Number(adultDraft) || 0) + 1))}><ChevronUp size={14} /></button><button type="button" aria-label="Decrease adults" disabled={Number(adultDraft) <= 1} onClick={() => setAdultDraft(String(Math.max(1, Math.floor(Number(adultDraft) || 0) - 1)))}><ChevronDown size={14} /></button></span></div></div><div className="booking-stepper"><div className="booking-stepper-row"><span className="booking-stepper-label">Child</span><input className="booking-stepper-value" type="number" min={0} step={1} required aria-label="No. of Child" value={childDraft} onChange={e => setChildDraft(e.target.value)} /><span className="booking-stepper-arrows"><button type="button" aria-label="Increase children" onClick={() => setChildDraft(String(Math.floor(Number(childDraft) || 0) + 1))}><ChevronUp size={14} /></button><button type="button" aria-label="Decrease children" disabled={Number(childDraft) <= 0} onClick={() => setChildDraft(String(Math.max(0, Math.floor(Number(childDraft) || 0) - 1)))}><ChevronDown size={14} /></button></span></div>{childAgePolicy > INFANT_MAX_AGE && <small className="booking-child-age-note">Age {INFANT_MAX_AGE + 1} - {childAgePolicy} years</small>}</div><div className="booking-stepper"><div className="booking-stepper-row"><span className="booking-stepper-label">Infant</span><input className="booking-stepper-value" type="number" min={0} step={1} required aria-label="No. of Infant" value={infantDraft} onChange={e => setInfantDraft(e.target.value)} /><span className="booking-stepper-arrows"><button type="button" aria-label="Increase infants" onClick={() => setInfantDraft(String(Math.floor(Number(infantDraft) || 0) + 1))}><ChevronUp size={14} /></button><button type="button" aria-label="Decrease infants" disabled={Number(infantDraft) <= 0} onClick={() => setInfantDraft(String(Math.max(0, Math.floor(Number(infantDraft) || 0) - 1)))}><ChevronDown size={14} /></button></span></div>{childAgePolicy > INFANT_MAX_AGE && <small className="booking-child-age-note">Age 0 - {INFANT_MAX_AGE} years</small>}</div></div>{error && <p role="alert">{error}</p>}<footer><button type="button" disabled={busy} onClick={() => setPaxOpen(false)}>Cancel</button><button disabled={busy} type="submit">Confirm</button></footer></form></DialogContent></Dialog>
    <div className="rooming-guest-card"><div className="rooming-table-head"><span>No.</span><span>Guest Name(s)</span></div>{assigned.length ? assigned.map((profile, index) => <div className="rooming-guest-row" key={profile.id}><strong>{index + 1}.</strong><div><strong><span className="billing-pax-tip" data-tip={profile.adultChild === 'Infant' ? 'Infant' : profile.adultChild === 'Child' ? 'Child' : 'Adult'}>{profile.adultChild === 'Infant' ? <InfantIcon size={17} /> : profile.adultChild === 'Child' ? <Baby size={17} /> : <PersonIcon size={17} />}</span> {profile.name}</strong><small>{profile.identityNo || profile.mobile || 'To Scan'}</small></div><button type="button" className="rooming-profile-button" aria-label={`Open ${profile.name} profile`} title="Open guest profile" onClick={() => editProfile(profile)}><span className="guest-photo-placeholder" /><ClipboardList size={15} /></button></div>) : <div className="rooming-empty">No guest profile is assigned yet. Add the first guest profile for this room.</div>}</div>
  ...
  return <section className="rooming-page" aria-label="Rooming list">
    ...
    {error && <p role="alert">{error}</p>}<div className="rooming-room-card"><div className="rooming-table-head rooming-room-head"><span>No.</span><span>Room Type</span><span><UserRound size={14} /></span><span>Guest Name(s)</span></div>{booking.rooms.flatMap((item, typeIndex) => { const guests = (item.guestProfileIds ?? []).map(id => profiles.find(profile => profile.id === id)).filter(Boolean) as GuestProfile[]; const offset = booking.rooms.slice(0, typeIndex).reduce((total, entry) => total + Math.max(1, entry.count ?? 1), 0); const numbers = assignedRoomNumbers[item.code] ?? []; return Array.from({ length: Math.max(1, item.count ?? 1) }, (_, copyIndex) => { const assignedRoomNo = numbers[offset + copyIndex]; return <div className="rooming-room-row" key={`${item.code}-${typeIndex}-${copyIndex}`}><strong>{offset + copyIndex + 1}.</strong><div><strong>{item.code}</strong><small>{item.rateCode ?? 'BAR'} Room Only</small>{assignedRoomNo ? <span className="rooming-room-badge">{assignedRoomNo}</span> : null}</div><span>{Math.max(1, (item.adults ?? booking.guests ?? 1) + (item.children ?? 0))}</span><div><strong>{guests.map(profile => profile.name).join(', ') || booking.guest}</strong><small>{guests.length ? `${guests.length} guest profile${guests.length === 1 ? '' : 's'}` : 'No guest profile assigned'}</small></div><button type="button" className="rooming-view-button" disabled={busy} aria-label={`View ${item.code} guest list`} title="View guest list" onClick={() => openGuests(typeIndex)}><img src="https://hms1.hotelx.asia/static/media/view_edit_icon.cb90a368.svg" alt="" aria-hidden="true" width={22} height={22} style={{ display: 'block', objectFit: 'contain' }} /></button></div>; }); })}</div>
  </section>;
}
