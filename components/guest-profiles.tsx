...
export function GuestProfiles({ profiles, onSave }: { profiles: GuestProfile[]; onSave: (value: GuestProfile[]) => Promise<void> }) {
  ...
  if (editing) { const states = geography.states[editing.country] || []; const cities = geography.cities[`${editing.country}|${editing.state}`] || []; const update = (key: keyof GuestProfile, value: string) => setEditing({ ...editing, [key]: value, ...(key === 'country' ? { state: '', city: '' } : key === 'state' ? { city: '' } : {}) } as GuestProfile); return <section className="guest-profile-page"><div className="guest-profile-banner"><button onClick={() => setEditing(null)}>‹</button><strong>Guest Profile</strong></div><div className="guest-profile-card"><h2>Guest Information</h2><div className="guest-profile-grid">{([['name','Guest Name *'],['mobile','Mobile No. (Optional)'],['email','Email Address *'],['nationality','Nationality *'],['identityNo','IC No / Passport No *'],['address','Address'],['country','Country *'],['state','State *'],['city','City'],['postcode','Postcode'],['birthDate','Birth Date'],['occupation','Occupation'],['accountName','Account Name (If applicable)'],['guestType','Guest Type *'],['adultChild','Adult/Child *'],['remark','Remark']] as const).map(([key,label]) => <label key={key}><span>{label}</span>{key === 'adultChild' ? <select value={editing.adultChild || 'Adult'} onChange={event => update(key, event.target.value)}><option>Adult</option><option>Child</option><option>Infant</option></select> : key === 'country' ? <select value={editing.country} onChange={event => update(key, event.target.value)}>{geography.countries.map(item => <option key={item}>{item}</option>)}</select> : key === 'state' ? <select value={editing.state} onChange={event => update(key, event.target.value)}><option value="">Select state</option>{states.map(item => <option key={item}>{item}</option>)}</select> : key === 'city' ? <select value={editing.city} onChange={event => update(key, event.target.value)}><option value="">Select city</option>{cities.map(item => <option key={item}>{item}</option>)}</select> : <input value={editing[key]} onChange={event => update(key, event.target.value)} />}</label>)}<label className="guest-toggle"><span>News Letter</span><input type="checkbox" checked={editing.newsletter} onChange={event => setEditing({ ...editing, newsletter: event.target.checked })} /></label><label className="guest-toggle"><span>Tourism Tax</span><input type="checkbox" checked={editing.tourismTax} onChange={event => setEditing({ ...editing, tourismTax: event.target.checked })} /></label></div></div><div className="guest-profile-actions"><button onClick={() => setEditing(null)}>Cancel</button><button className="primary-button" onClick={save}>Save</button></div></section>; }
