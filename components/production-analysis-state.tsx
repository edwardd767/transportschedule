...
function stateOf(booking: Booking, profiles: GuestProfile[]) {
  const ids = new Set(booking.rooms.flatMap((room) => room.guestProfileIds ?? []));
  for (const id of ids) {
    const profile = profiles.find((item) => item.id === id);
    if (profile?.state) return profile.state.toUpperCase();
  }
  return 'BLANK';
}
...
function buildRows(bookings: Booking[], profiles: GuestProfile[], date: string) {
  ...
  for (const booking of bookings.filter((item) => overlaps(item, startOfYear(date), date) && !['Cancelled', 'No Show'].includes(item.status))) {
    const state = stateOf(booking, profiles);
    groups.set(state, [...(groups.get(state) ?? []), booking]);
  }
...
export function ProductionAnalysisState({ bookings, profiles, hotelMasters, date, onDate, onBack }: {
  ...
  return <div className="production-analysis-report">
    ...
    <header className="production-analysis-heading"><div>{hotelMasters.profile.hotelName}</div><h2>Production Analysis by State/Country/Nationality</h2><p>{dateLabel(date)}</p></header>
    <div className="production-analysis-wrap"><table className="production-analysis-table"><thead><tr><th rowSpan={2} scope="col">State</th>{periods.map(scope => <th key={scope} colSpan={metricColumns.length} scope="colgroup">{scope === 'today' ? 'TODAY' : scope === 'month' ? 'MONTH TO DATE' : 'YEAR TO DATE'}</th>)}</tr><tr>{periods.flatMap(scope => metricColumns.map(column => <th key={`${scope}-${column}`} scope="col">{column}</th>))}</tr></thead><tbody>{rows.map(row => <StateRow key={row.state} label={row.state} cells={row.cells} />)}{!rows.length && <tr><td colSpan={metricColumns.length * periods.length + 1}>No booking data found for this report date.</td></tr>}<StateRow label="Total" cells={totalCells} total /><StateRow label="Grand Total" cells={totalCells} total /></tbody></table></div>
