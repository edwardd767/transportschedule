...
function salesChannelOf(booking: Booking) {
...
function SalesChannelRow({ channel, cells, total }: { channel: string; cells: (number | string)[]; total?: boolean }) {
  return <tr className={total ? 'production-total-row' : undefined}><th scope="row">{channel}</th>{cells.map((value, index) => { const format = cellFormats[index % metricColumns.length]; return <td key={index}>{format === 'money' ? money(value as number) : format === 'text' ? value : number(value as number)}</td>; })}</tr>;
}
