...
function ValidityEditor({ item, validity, seasons, roomTypes, elements, addOns, onCancel, onSave }: { item: RatePlanItem; validity: RateValidityItem; seasons: Season[]; roomTypes: { code: string; description: string }[]; elements: RateElementItem[]; addOns: AddOnItem[]; onCancel: () => void; onSave: (value: RateValidityItem) => void | Promise<void> }) {
...
function RateSetupPage({ items, rateTypes, validityItems, seasons, roomTypes, elements, addOns, onChange, onValidityChange }: { items: RatePlanItem[]; rateTypes: RateTypeItem[]; validityItems: RateValidityItem[]; seasons: Season[]; roomTypes: { code: string; description: string }[]; elements: RateElementItem[]; addOns: AddOnItem[]; onChange: (value: RatePlanItem[]) => void | Promise<void>; onValidityChange: (value: RateValidityItem[]) => void | Promise<void> }) {
  const [menuId, setMenuId] = useState<string | null>(null);
  const [query, setQuery] = useState('');
...
export function RateSetupModule({
  ...
  const savePart = <K extends keyof RateSetupData>(key: K, value: RateSetupData[K]) => {
