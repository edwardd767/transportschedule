...
function SeasonRateDialog({ roomName, season, value, onCancel, onConfirm }: { roomName: string; season: Season; value: { amount: number; t1: number; t2: number; t3: number; quotas?: number[] }; onCancel: () => void; onConfirm: (value: { amount: number; t1: number; t2: number; t3: number; quotas?: number[] }) => void }) {
  ...
  const valid = amount.trim() !== '' && Number.isFinite(Number(amount)) && Number(amount) >= 0 && tiers.every(t => t.quota.trim() !== '' && Number.isInteger(Number(t.quota)) && Number(t.quota) >= 0 && t.rate.trim() !== '' && Number.isFinite(Number(t.rate)) && Number(t.rate) >= 0);
  return <div className="rate-element-overlay"><form className="season-dialog" role="dialog" aria-modal="true" aria-label="Edit seasonal rate" onSubmit={event => { event.preventDefault(); if(valid) onConfirm({ amount: Number(amount), t1: Number(tiers[0]?.rate ?? 0), t2: Number(tiers[1]?.rate ?? 0), t3: Number(tiers[2]?.rate ?? 0), quotas: tiers.map(t => Number(t.quota)) }); }}>
    ...
    <label className="season-standard"><strong>Standard Rate</strong><input aria-label="Standard Rate" type="number" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} /></label>
