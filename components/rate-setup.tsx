...
function ValidityEditor({ item, validity, seasons, roomTypes, elements, onCancel, onSave }: { item: RatePlanItem; validity: RateValidityItem; seasons: Season[]; roomTypes: { code: string; description: string }[]; elements: RateElementItem[]; onCancel: () => void; onSave: (value: RateValidityItem) => void }) {
  const [draft, setDraft] = useState(validity);
  const [inclusiveOpen, setInclusiveOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'seasonal' | 'inclusive'>('seasonal');
  const [elementQuery, setElementQuery] = useState('');
  const selectedElements = draft.inclusiveElements ?? [];
  const saveDraft = () => onSave({ ...draft, inclusiveElements: selectedElements, seasonalRates: { ...draft.seasonalRates, __inclusiveElements: selectedElements } as RateValidityItem['seasonalRates'] });
...
function RateSetupPage({ items, rateTypes, validityItems, seasons, roomTypes, elements, onChange, onValidityChange }: { items: RatePlanItem[]; rateTypes: RateTypeItem[]; validityItems: RateValidityItem[]; seasons: Season[]; roomTypes: { code: string; description: string }[]; elements: RateElementItem[]; onChange: (value: RatePlanItem[]) => void | Promise<void>; onValidityChange: (value: RateValidityItem[]) => void | Promise<void> }) {
  ...
  if (validity && validityPage) return <ValidityEditor item={validityPage} validity={validity} seasons={seasons} roomTypes={roomTypes} elements={elements} onCancel={() => setValidity(null)} onSave={(value) => { void onValidityChange([...validityItems.filter((row) => row.id !== value.id), value]); setValidity(null); }} />;
