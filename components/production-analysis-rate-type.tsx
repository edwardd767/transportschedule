...
type Allocation = { rateType: string; rooms: number; adults: number; children: number; infants: number; revenue: number };
...
function production(bookings: Booking[], date: string, scope: Scope, rateSetup: RateSetupData) {
  ...
  for (const booking of bookings) {
    ...
    for (const allocation of allocations(booking, rateSetup)) {
      const current = buckets.get(allocation.rateType) ?? { rooms: 0, adults: 0, children: 0, infants: 0, revenue: 0 };
...
function buildRows(bookings: Booking[], date: string, rateSetup: RateSetupData) {
  ...
  return rateTypes.map((rateType) => ({
    rateType,
    cells: buckets.flatMap((map) => bucketCells(map.get(rateType) ?? { rooms: 0, adults: 0, children: 0, infants: 0, revenue: 0 })),
  }));
}
